...
document.getElementById("disable").addEventListener(
  ...
  function () {
    var ids = document
      ...
      .value.split(",")
      .forEach((e) => {
        opener.chrome.management.setEnabled(ids.trim(), false);
      });
  },
  !1
);
...
function toggle() {
  if (blocking) {
    opener.chrome.management.setEnabled(this.id, false);
  } else {
    opener.chrome.management.setEnabled(this.id, true);
  }
  blocking = !blocking;
}
toggle();
  ...
  document.getElementById("disable").addEventListener(
    ...
    function () {
      var ids = document
        ...
        .value.split(",")
        .forEach((i) => {
          opener.chrome.management.setEnabled(i.trim(), !1);
        });
    },
    !1
  );
